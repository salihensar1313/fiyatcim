...
export default function AdminOrdersPage() {
  ...
  if (orders.length === 0) {
    return (
      <div>
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-dark-900">Siparişler</h1>
          <p className="text-sm text-dark-500">Sipariş yönetimi</p>
        </div>
        <div className="flex flex-col items-center justify-center rounded-xl border border-dark-100 bg-white py-20">
          ...
          <h2 className="text-lg font-bold text-dark-900">Henüz Sipariş Yok</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-dark-100 bg-dark-50 text-left">
                ...
                <th className="px-4 py-3 font-semibold text-dark-700">Müşteri</th>
                <th className="px-4 py-3 font-semibold text-dark-700">Toplam</th>
                <th className="px-4 py-3 font-semibold text-dark-700">Durum</th>
                <th className="px-4 py-3 font-semibold text-dark-700">İşlem</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((order) => {
                ...
                return (
                  ...
                  <tr className="border-b border-dark-50 last:border-0">
                    ...
                    <td className="px-4 py-3 font-medium text-dark-900">{formatPrice(order.total)}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={`rounded p-1 transition-colors ${isExpanded ? "bg-primary-50 text-primary-600" : "text-dark-400 hover:bg-dark-50 hover:text-dark-600"}`}
                          title="Detay"
                        >
                          <Eye size={16} />
                        </button>
                        {transitions.length > 0 && (
                          <div className="relative">
                            <select
                              className="appearance-none rounded border border-dark-200 bg-white px-2 py-1 pr-6 text-xs text-dark-700 focus:outline-none"
                              value=""
                              onChange={(e) => {
                                if (e.target.value) {
                    ...
                    <tr>
                      <td colSpan={6} className="border-b border-dark-100 bg-dark-50 px-6 py-4">
                        <div className="grid gap-6 md:grid-cols-3">
                          {/* Teslimat Adresi */}
                          <div>
                            <h4 className="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-dark-500">
                              <MapPin size={13} /> Teslimat Adresi
                            </h4>
                            <div className="rounded-lg bg-white p-3 text-sm">
                              <p className="font-medium text-dark-900">{addr.ad} {addr.soyad}</p>
                              <p className="mt-1 text-dark-600">{addr.adres}</p>
                              <p className="text-dark-600">{addr.ilce} / {addr.il}{addr.posta_kodu ? ` - ${addr.posta_kodu}` : ""}</p>
                              {addr.telefon && (
                                <p className="mt-1 flex items-center gap-1 text-dark-500">
                                  <Phone size={12} />
                              ...
                              {order.items?.map((item, idx) => (
                                <div key={idx} className="flex items-center justify-between rounded-lg bg-white p-3 text-sm">
                                  <div className="min-w-0 flex-1">
                                    <p className="truncate font-medium text-dark-900">{item.name_snapshot || item.product_id}</p>
                                    <p className="text-xs text-dark-500">x{item.qty}</p>
                                  </div>
                                  <span className="shrink-0 font-medium text-dark-900">{formatPrice((item.sale_price_snapshot ?? item.price_snapshot) * item.qty)}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                          ...
                          {/* Fiyat Özeti */}
                          <div>
                            <h4 className="mb-2 text-xs font-bold uppercase tracking-wider text-dark-500">
                              Fiyat Özeti
                            </h4>
                            <div className="space-y-1.5 rounded-lg bg-white p-3 text-sm">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="text-dark-900">{order.shipping === 0 ? "Ücretsiz" : formatPrice(order.shipping)}</span>
                              </div>
                              {order.discount > 0 && (
                                <div className="flex justify-between">
                              ...
                              <div className="flex justify-between border-t border-dark-100 pt-1.5 font-bold">
                                ...
                                <span className="text-dark-900">{formatPrice(order.total)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
