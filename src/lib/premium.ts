/**
 * Premium Üyelik — Fiyatcim.com
 *
 * Premium üyeler: Ücretsiz profesyonel kurulum desteği alır.
 * Fiyat:
 *   - Sepette (ürün alırken): ₺2.500
 *   - Harici (sadece premium): ₺3.000
 */

export const PREMIUM_PRICE_WITH_ORDER = 2500;
export const PREMIUM_PRICE_STANDALONE = 3000;

export const PREMIUM_BENEFITS = [
  {
    icon: "Wrench" as const,
    title: "Ücretsiz Profesyonel Kurulum Desteği",
    desc: "Uzman ekibimiz tüm ürünlerinizi ücretsiz kurar",
    highlight: true,
  },
  {
    icon: "Headphones" as const,
    title: "7/24 Öncelikli Destek",
    desc: "Premium destek hattına sınırsız erişim",
  },
  {
    icon: "Shield" as const,
    title: "Genişletilmiş Garanti",
    desc: "Standart garantiye ek +1 yıl uzatılmış garanti",
  },
  {
    icon: "Truck" as const,
    title: "Tüm Siparişlerde Ücretsiz Kargo",
    desc: "Tutar fark etmeksizin tüm kargolar ücretsiz",
    highlight: true,
  },
  {
    icon: "Zap" as const,
    title: "Aynı Gün Kargo",
    desc: "Siparişleriniz aynı gün kargoya verilir",
  },
  {
    icon: "Crown" as const,
    title: "Altın Premium Tema",
    desc: "Özel altın navigasyon, avatar ve butonlar ile VIP deneyim",
    highlight: true,
  },
  {
    icon: "Palette" as const,
    title: "Premium Profil Rozeti",
    desc: "Hesabınızda ve yorumlarınızda altın Premium rozet",
  },
  {
    icon: "Tv" as const,
    title: "1 Ay Ücretsiz Netflix",
    desc: "Premium üyelere hediye 1 aylık Netflix aboneliği",
  },
  {
    icon: "Music" as const,
    title: "1 Ay Ücretsiz Spotify",
    desc: "Premium üyelere hediye 1 aylık Spotify Premium",
  },
  {
    icon: "Settings" as const,
    title: "Uzaktan Erişim Kurulumu",
    desc: "Mobil uygulama ve uzaktan izleme ayarları yapılır",
  },
  {
    icon: "RefreshCw" as const,
    title: "Yıllık Bakım Ziyareti",
    desc: "Yılda 1 kez ücretsiz sistem kontrolü ve bakım",
  },
  {
    icon: "Bell" as const,
    title: "Kampanyalardan İlk Haberdar",
    desc: "Yeni ürünler ve indirimleri herkesten önce görün",
  },
];
